//
//  ReportExportTeacherConfidenceInTechView.swift
...
struct ReportExportTeacherConfidenceInTechView: View {
    ...
    var body: some View {
        GridRow {
            let technologiesUsed = Array(Set(session.observations.compactMap {
                $0.teacherConfidenceInTechnology?.dictValue?.keys.map({ $0 })
            }.flatMap { $0 })).sorted()
                ...
                HStack {
                    ...
                    Grid(horizontalSpacing: 0, verticalSpacing: 0) {
                        ...
                        ForEach(technologiesUsed, id: \.self) { technology in
                            GridRow {
                                ForEach(Array(session.sortedObservations.reversed())) { observation in
                                    let observationDict = observation.teacherConfidenceInTechnology?.dictValue ?? [:]
